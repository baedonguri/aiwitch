...
- **`login`** — interactive ChatGPT or API key login with hardened secret entry (volatile zeroing of the key buffer).
...
- CI on GitHub Actions: `fmt`, `clippy`, matrix `test` (ubuntu, macos), and `doc` jobs — all `--locked` and `--all-features`, with `RUSTDOCFLAGS=-D warnings`.
